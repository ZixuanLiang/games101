...
{   
    ...
    for (int i = 0; i < 3; i++) {
        Vector3f a(_v[i].x(), _v[i].y(), 0);
        Vector3f b(_v[(i+1) % 3].x(), _v[(i+1) % 3].y(), 0);
        Vector3f ab = b - a;
        Vector3f p(x, y, 0);
        Vector3f ap = p - a;
        curr_sign = ab.cross(ap).z();
        if (curr_sign * prev_sign > 0){
            continue;
        } else if (curr_sign * prev_sign == 0)
        {
            prev_sign = curr_sign;
        } else {
            return false;
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    int x_min = std::min(t.v[0].x(), std::min(t.v[1].x(), t.v[2].x())), x_max = std::max(t.v[0].x(), std::max(t.v[1].x(), t.v[2].x()));
    int y_min = std::min(t.v[0].y(), std::min(t.v[1].y(), t.v[2].y())), y_max = std::max(t.v[0].y(), std::max(t.v[1].y(), t.v[2].y()));
